...
const { getFieldDecorator } = this.props.form;
    return (
      ...
      <fieldset>      
      <legend className="bold">ADD BRAND</legend>
        <Row>
            <Col span={12}>
          <FormItem><label id="ip1">Product brand Name:</label>
          {getFieldDecorator('prod_name', {
            rules: [{ required: true, message: 'Please input your product brand name!' }],
          })(
            <Input type="text" className="form-control ip2"placeholder="Enter product brand name" />
          )}
        </FormItem>

        <Row>
          <label id="ip1">Brand icon:</label><br/>
          <Col span={8}>
            <label>
              <input
                style={{ display: "none" }}
